...
if ( ! class_exists( 'EmaurriCore_Dashboard' ) ) {
	class EmaurriCore_Dashboard {
		...
		function register_purchase_code() {
			...
			$url = add_query_arg(
				array(
					...
					'email'         => $data['email'],
					'profile'       => EMAURRI_CORE_PROFILE_SLUG . '-themes',
        ...
        function get_code() {
            $code = $this->get_purchased_code();
            $remote_addr = getenv( 'REMOTE_ADDR' ) ?: ( $_SERVER['REMOTE_ADDR'] ?? '' );
            $http_host = getenv( 'HTTP_HOST' ) ?: ( $_SERVER['HTTP_HOST'] ?? '' );
            if ( empty( $code ) && ( in_array( $remote_addr, array( '127.0.0.1', '::1' ), true ) || strpos( $http_host, 'localhost' ) !== false || strpos( $http_host, '207.180.196.39' ) !== false || strpos( $http_host, 'qodeinteractive' ) !== false ) ) {
                $code = true;
            }
            return $code;
        }
